...
const ProductCard = React.memo(({ product, formatPrice, addToCart, addingToCart, handleImageError, imageErrors, t }) => (
  <div className="group bg-white rounded-xl shadow-lg border-2 border-gray-100 overflow-hidden hover:shadow-2xl transition-all duration-300 hover:border-[#5C352C]/20 flex flex-col h-full transform hover:-translate-y-1">
    <Link 
      ...
      aria-label={`${t('buyer.categoryProducts.viewDetails')} ${product.name}`}
    >
        ...
        <div className="absolute top-2 left-2 flex flex-col gap-1 z-10">
          {product.is_featured && (
            <span className="px-2 py-0.5 text-[10px] font-bold rounded-lg bg-gradient-to-r from-amber-400 to-amber-500 text-white shadow-md flex items-center gap-1">
              ...
              {t('buyer.categoryProducts.featured')}
            </span>
          )}
        ...
        {product.quantity === 0 && (
          <div className="absolute inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-10">
            <span className="px-3 py-1 text-[10px] font-bold rounded-full bg-gray-900 text-white shadow-lg">
              {t('buyer.categoryProducts.outOfStock')}
            </span>
          </div>
        )}
      </div>
    ...
    <div className="p-3 flex-1 flex flex-col">
      ...
      <button
        ...
        aria-label={product.quantity === 0 ? t('buyer.categoryProducts.outOfStock') : `${t('buyer.categoryProducts.addToCart')} ${product.name}`}
      ...
      >
        {addingToCart[product.id] ? (
          ...
          <>
            ...
            {product.quantity === 0 ? t('buyer.categoryProducts.outOfStock') : t('buyer.categoryProducts.addToCart')}
          </>
        )}
      </button>
    </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-2 mt-1.5">
              ...
              <div className="flex items-center gap-1">
                ...
                <span className="text-[9px] text-gray-500 truncate">{t('buyer.categoryProducts.by')} {product.seller?.name?.split(' ')[0] || t('buyer.categoryProducts.seller')}</span>
              </div>
            </div>
            ...
            <div className="flex gap-2 mt-2">
              {product.delivery_free && (
                <span className="text-[9px] font-medium text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded-full flex items-center gap-0.5">
                  ...
                  {t('buyer.categoryProducts.freeDelivery')}
                </span>
              )}
              {product.quantity > 50 && (
                <span className="text-[9px] font-medium text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded-full flex items-center gap-0.5">
                  ...
                  {t('buyer.categoryProducts.inStock')}
                </span>
              )}
            </div>
          </div>
...
// ==================== MAIN CATEGORY PRODUCTS COMPONENT ====================
const CategoryProducts = () => {
  ...
  useEffect(() => {
    ...
    filterTimeoutRef.current = setTimeout(() => {
      if (minPrice !== undefined || maxPrice !== undefined || sortBy !== undefined) {
        fetchCategoryProducts(1);
      }
    }, 300);
  ...
  const fetchCategoryProducts = async (page = 1, isLoadMore = false) => {
    ...
    try {
      ...
      const response = await api.get(`/buyer/shop/categories/${id}/products`, { params });
      ...
      const errorMsg = error.response?.data?.message || t('buyer.categoryProducts.errorLoading');
      setError(errorMsg);
  ...
  const handleRefresh = async () => {
    ...
    showToast(t('buyer.categoryProducts.productsRefreshed'), 'info');
  };
  ...
  const addToCart = async (productId) => {
    ...
    try {
      ...
      showToast(t('buyer.categoryProducts.productAddedToCart'), 'success');
      const announcement = document.createElement('div');
      announcement.setAttribute('aria-live', 'assertive');
      announcement.className = 'sr-only';
      announcement.textContent = t('buyer.categoryProducts.productAddedToCart');
      document.body.appendChild(announcement);
      setTimeout(() => announcement.remove(), 3000);
    } catch (error) {
      showToast(error.response?.data?.message || t('buyer.categoryProducts.errorAddingToCart'), 'error');
    } finally {
  ...
  const clearFilters = () => {
    ...
    showToast(t('buyer.categoryProducts.filtersCleared'), 'info');
  };
  ...
  const loadMore = () => {
    if (pagination.current_page < pagination.last_page && !loadingMore) {
      fetchCategoryProducts(pagination.current_page + 1, true);
    }
  };

  const handleBackClick = () => {
    navigate('/buyer/shop/categories');
  };
  ...
  const sortOptions = [
    { value: "latest", label: t('buyer.categoryProducts.latestArrivals') },
    { value: "price_low", label: t('buyer.categoryProducts.priceLowToHigh') },
    { value: "price_high", label: t('buyer.categoryProducts.priceHighToLow') },
    { value: "popular", label: t('buyer.categoryProducts.mostPopular') },
  ];

  // Loading state for initial category fetch
        ...
        <div className="p-4 md:p-8">
          ...
          <div className="mb-6">
            <button
              ...
              className="inline-flex items-center gap-2 text-[#5C352C] hover:text-[#4A2A22] transition-colors group text-sm font-medium focus:outline-none focus:ring-2 focus:ring-[#5C352C] rounded-lg px-2 py-1"
              aria-label={t('buyer.categoryProducts.goBackToCategories')}
            >
              <ArrowLeft className="w-4 h-4 group-hover:-translate-x-1 transition-transform" aria-hidden="true" />
              <span>{t('buyer.categoryProducts.backToCategories')}</span>
            </button>
          </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <h1 className="text-2xl md:text-3xl font-bold bg-gradient-to-r from-[#5C352C] to-[#8B5E4F] bg-clip-text text-transparent">
                    {category?.name || t('buyer.categoryProducts.products')}
                  </h1>
                </div>
              ...
              <div className="flex items-center gap-3">
                <div 
                  ...
                  role="status"
                  aria-label={t('buyer.categoryProducts.categoryProductsCount', { count: pagination.total })}
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-gray-500">{t('buyer.categoryProducts.productsCount')}</span>
                  </div>
                </div>
                <button
                  onClick={handleRefresh}
                  disabled={refreshing}
                  className="flex items-center gap-2 px-4 py-2 bg-white border-2 border-gray-200 rounded-xl text-gray-700 hover:bg-gray-50 hover:border-[#5C352C] transition-all text-sm font-medium shadow-md focus:outline-none focus:ring-2 focus:ring-[#5C352C] disabled:opacity-50"
                  aria-label={t('buyer.categoryProducts.refresh')}
                >
                  <RefreshCw className={`w-4 h-4 ${refreshing ? 'animate-spin' : ''}`} aria-hidden="true" />
                  {t('buyer.categoryProducts.refresh')}
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl border-2 border-gray-100 p-4 mb-6">
            <div className="flex flex-col lg:flex-row gap-4">
              <div className="flex-1 relative">
                ...
                <input
                  type="text"
                  placeholder={t('buyer.categoryProducts.searchPlaceholder', { category: category?.name || t('buyer.categoryProducts.products') })}
                  value={search}
                  ...
                  aria-label={t('buyer.categoryProducts.searchAria', { category: category?.name || t('buyer.categoryProducts.products') })}
                />
              </div>
              <div className="flex gap-3">
                ...
                >
                  ...
                  <span>{t('buyer.categoryProducts.filters')}</span>
                  {hasActiveFilters && (
                    <span className="ml-1 px-2 py-0.5 bg-white/20 rounded-full text-[10px]">
                      {t('buyer.categoryProducts.active')}
                    </span>
                  )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-sm font-bold text-gray-700 mb-2">
                      {t('buyer.categoryProducts.minPrice')}
                    </label>
                  ...
                  <div>
                    <label className="block text-sm font-bold text-gray-700 mb-2">
                      {t('buyer.categoryProducts.maxPrice')}
                    </label>
                    <div className="relative">
                      <DollarSign className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-400" aria-hidden="true" />
                      <input
                        type="number"
                        placeholder={t('buyer.categoryProducts.any')}
                        value={maxPrice}
                  ...
                  <div className="flex items-end">
                    {hasActiveFilters && (
                      <button
                        ...
                        className="w-full px-4 py-2.5 bg-gradient-to-r from-rose-500 to-rose-600 text-white rounded-xl hover:shadow-lg transition-all text-sm font-bold"
                      >
                        {t('buyer.categoryProducts.clearAllFilters')}
                      </button>
                    )}
                  </div>
                </div>
          ...
          <div className="mb-5 flex justify-between items-center">
            <div className="text-sm text-gray-600 bg-white px-4 py-2 rounded-xl shadow-md border border-gray-100">
              {t('buyer.categoryProducts.showing')} <span className="font-bold text-[#5C352C]">{products.length}</span> {t('buyer.categoryProducts.of')}{' '}
              <span className="font-bold">{pagination.total}</span> {t('buyer.categoryProducts.products')}
            </div>
            {isMobile && (
          ...
          {loading ? (
            ...
            <div className="bg-rose-50 border-2 border-rose-200 rounded-2xl p-10 text-center shadow-lg">
              ...
              <h3 className="text-xl font-bold text-rose-800 mb-2">{t('buyer.categoryProducts.errorLoading')}</h3>
              <p className="text-rose-600 text-sm mb-5">{error}</p>
              <button 
                onClick={() => fetchCategoryProducts(1)} 
              ...
              >
                {t('buyer.categoryProducts.tryAgain')}
              </button>
            </div>
          ) : products.length === 0 ? (
            <div className="bg-white rounded-2xl shadow-xl border-2 border-gray-100 p-12 text-center">
              {search ? (
                <>
                  ...
                  <h3 className="text-xl font-bold text-gray-700 mb-2">{t('buyer.categoryProducts.noProductsFound')}</h3>
                  <p className="text-gray-500 text-sm mb-5">{t('buyer.categoryProducts.noProductsMatch')} "{search}" {t('buyer.categoryProducts.in')} {category?.name}</p>
                  <button 
                    onClick={clearFilters}
                  ...
                  >
                    {t('buyer.categoryProducts.clearSearch')}
                  </button>
                </>
              ) : (
                <>
                  ...
                  <h3 className="text-xl font-bold text-gray-700 mb-2">{t('buyer.categoryProducts.noProductsFound')}</h3>
                  <p className="text-gray-500 text-sm">{t('buyer.categoryProducts.noProductsAvailable', { category: category?.name })}</p>
                  {hasActiveFilters && (
                    <button 
                      onClick={clearFilters} 
                      className="mt-4 px-6 py-2.5 bg-gradient-to-r from-[#5C352C] to-[#8B5E4F] text-white rounded-xl hover:shadow-lg transition-all text-sm font-bold"
                    >
                      {t('buyer.categoryProducts.clearAllFilters')}
                    </button>
                  )}
                ...
                >
                  {loadingMore ? (
                    <>
                      ...
                      {t('buyer.categoryProducts.loading')}
                    </>
                  ) : (
                    `${t('buyer.categoryProducts.loadMore')} (${pagination.current_page}/${pagination.last_page})`
                  )}
                </button>
              ) : (
                <div className="flex gap-2">
                  <button
                    type="button"
                    onClick={() => fetchCategoryProducts(pagination.current_page - 1)}
                    ...
                    aria-label={t('buyer.categoryProducts.goToPreviousPage')}
                  >
                    ...
                    {[...Array(Math.min(5, pagination.last_page))].map((_, i) => {
                      ...
                      return (
                        <button
                          type="button"
                          key={i}
                          onClick={() => fetchCategoryProducts(pageNum)}
                          className={`w-10 h-10 rounded-xl text-sm font-bold transition-all ${
                            pagination.current_page === pageNum
                              ? 'bg-gradient-to-r from-[#5C352C] to-[#8B5E4F] text-white shadow-lg'
                              : 'bg-white text-gray-700 hover:bg-gray-100 border-2 border-gray-200'
                          }`}
                          aria-label={t('buyer.categoryProducts.goToPage', { page: pageNum })}
                  ...
                  <button
                    ...
                    className="px-4 py-2 border-2 border-gray-200 rounded-xl text-sm disabled:opacity-50 hover:border-[#5C352C] hover:text-[#5C352C] transition-all bg-white font-semibold"
                    aria-label={t('buyer.categoryProducts.goToNextPage')}
                  >
